# Incremental Dataset Stitching Plan

This document turns the model timeline draft into a CAIDBench data-stitching
...
Every stitched sample should normalize into the existing CAIDBench sidecar
schema:
...
| `split` | Normalize to `train`, `val`, and `test`. If an upstream source only has validation data used for evaluation, copy or map that split to `test` in the stitched package. |
...
Before using the stitched dataset for experiments, check:
...
Train with the stitched timeline protocol after the combined Arrow dataset is
available:
