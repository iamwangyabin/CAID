...
Use `method.name=ranpac` with `configs/reproduce/ranpac.yaml`. CAIDBench follows the official CDDB full RanPAC row (`ID=7` in `args/cddb_publish.csv`): first-task AdaptFormer-style adapter tuning on ViT-B/16 IN21k with the official CosineLinear head for 20 epochs, SGD with `body_lr=head_lr=0.01`, `batch_size=48`, `weight_decay=0.0005`, `min_lr=0.0`, then frozen-feature random projection and cumulative ridge with `M=5000`, no input normalization, and the official ridge grid `1e-8...1e8`. The detector keeps native 768-D ViT features (`detector_cfg.backbone.out_dim=null` so the base config's 512-D projection is disabled).

The first-task adapter tuning uses the official training transform `RandomResizedCrop(224, scale=(0.05, 1.0))`, horizontal flip, `ToTensor`, and AdaptFormer adapter dropout `0.1`. Ridge statistics are collected from the same train split with the test transform `Resize(256)+CenterCrop(224)+ToTensor`, matching the official `train_loader_for_CPs` with `mode="test"`. For the no-PETL Phase-2 ablation (`ID=4`: `model_name=ncm`, `batch_size=128`, `tuned_epoch=0`), use `configs/reproduce/ranpac_no_petl.yaml`.
...
Use `method.name=pina` or `method.name=pina_d` with `configs/reproduce/pina.yaml`. The current CAIDBench module is a feature-space approximation: a frozen feature extractor, one residual domain adapter per task, a unified classifier trained on the base task then frozen, K-Means task keys, and routed inference. `pina` maps to shallow feature adapters, while `pina_d` maps to the deeper adapter path. The compact path now follows the released code's cosine schedule and L1 task-center routing, but it does not duplicate token-level ViT prompts, per-block DSA injection, or the CLIP text-prompt classifier. The released `utils/pss.py` patch-shuffle helper is present in the official repo but is not wired into the published training/eval path.
...
Use `method.name=cp_prompt` with `configs/reproduce/cp_prompt.yaml`. The default config now uses `implementation=official`: an OpenAI CLIP ViT-B/16 path with the released Prefix-One mechanics, including shared instance tokens inserted after the class token, per-task K/V prefix prompts on the configured visual transformer layers, one CoOp-style CLIP text prompt learner per task, `logit_scale` cosine classification, frozen CLIP weights, current-task-only text prompt training, shared/prefix prompt training, task-center routing with the official L1 distance, and fixed shared-prompt snapshots when `is_fix_share_prompt=true`. Install optional CLIP dependencies with `pip install -e .[clip]` or point `method.detector_cfg.backbone.pretrained_path` at a local OpenAI CLIP checkpoint. `implementation=feature_space` is still available as a compact approximation for smoke tests, but it is not paper-number equivalent.
...
Use `method.name=duct` with `configs/reproduce/duct.yaml`. CAIDBench follows the released DUCT control flow: dynamically expand a method-local cosine head, train only the current domain slice, reset the backbone to the initial state before each task, compute class means with the initial backbone, train split, evaluation transform, and no dropped tail batch, merge task vectors with `merge_scalar`, retrain the expanded head, and apply Sinkhorn OT transport from the current slice to old slices using class means. Prediction folds expanded logits back to CAIDBench's binary metric space by taking the best real/fake logit across seen domains. `configs/reproduce/duct.yaml` disables the base config's 512-D projection so ViT-B/16 keeps the official 768-D feature dimension, pins timm pretraining to the Google augreg `.npz` URL used by the official repo, and keeps the retrain head learning rate at the official code's effective parameter-group LR (`lrate`) even though the released config also defines `lr_re`. The trainer also logs DUCT-paper-style sample-weighted seen-set accuracy as `official_average_accuracy`/`official_last_accuracy`, because the official table's `Ā`/`AB` are not the unweighted CAIDBench task-matrix mean. The wrapper is still CAIDBench-native rather than a vendored copy of official `IncViTCosine`, so exact paper-number runs should use the same CDDB image tree/split as the official repository.
...
Use `method.name=soyo` with `configs/reproduce/soyo.yaml`. The default config now uses `implementation=official` for the released SOYO-ViT path: a frozen timm ViT receives per-domain learned prompt tokens, each transformer block applies the current domain's down/up offset module, only the current `prompt_pool`/`classifier`/`down_pool`/`up_pool` parameters are trainable, each domain's normalized prompt-free image features are compressed with a two-component full-covariance GMM, and a linear SOYO selector is trained from current-domain features plus old GMM-resampled features before routing inference through the selected domain parameters. `implementation=feature_space` is still available for CPU smoke tests and uses the older adapter/classifier approximation. The official `soyo_clip` text-prompt path is not yet implemented in CAIDBench.
...
Use `method.name=loranpac` with `configs/reproduce/loranpac.yaml`. CAIDBench follows the official low-rank random-feature solver: random features `H = relu(F @ RE)`, cumulative `cov_HY`, the QR-based incremental truncated SVD summary of `H^T`, and on-the-fly ridge weights `U @ ((U.T @ cov_HY) / (Sigma^2 + ridge))`. The local solver keeps rank according to cumulative sample count, matching the official `round(total_samples * (1 - truncate_percent/100))` behavior, buffers feature chunks until the official 10000-sample update threshold, then flushes the final chunk at task end. The LoRanPAC config uses native 768-D ViT-B/16 features and extracts TSVD features from the train split with the test transform, matching the official `source="train", mode="test"` loader.
...
Use `method.name=dce` with `configs/reproduce/dce.yaml`. CAIDBench implements the DCE expert group as three per-domain expert heads: naive CE, balanced softmax, and reverse/few-shot CE. The official-compatible path uses cosine heads with learnable scale and official initialization, reinitializes the MLP selector to `3 * num_seen_tasks` outputs at each task, uses CosineAnnealing schedules, domain-level class priors for balanced/reverse losses, ViT prompt-token insertion for timm ViT backbones when `prompt_type=one/all`, test-transform train features for class statistics, official-style task-shared OAS covariance estimates, and fresh Gaussian pseudo-feature replay each selector epoch with the released feature-scaling default. The official code's CDDB-Hard imbalance table differs from the paper appendix; reproduce official-code numbers by following `utils/data.py::make_imb()` and recording that choice in experiment metadata.
...
The default `configs/reproduce/prompt2guard.yaml` reads the official object-label sidecar from `data/sidecars/prompt2guard/classes.pkl`. Download it before running Prompt2Guard:
...
`configs/reproduce/sur_lid.yaml` requires the official EfficientNet-B4 checkpoint by default. Place it at `./training/pretrained/efficientnet-b4-6ed6700e.pth`, update `method.detector_cfg.backbone.pretrained_path`, or set `method.require_backbone_checkpoint=false` for non-paper smoke checks.
