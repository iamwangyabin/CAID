...
Use `method.name=duct` with `configs/duct.yaml`. CAIDBench now keeps a method-local domain-expanded cosine head, trains only the current domain slice, resets the backbone to the initial state before each task, merges task vectors with `merge_scalar`, retrains the expanded head, and applies a lightweight Sinkhorn OT transport from the current slice to old slices using class means. Prediction folds expanded logits back to CAIDBench's binary metric space by taking the best real/fake logit across seen domains. This matches the released DUCT control flow more closely than the previous shared-head interpolation, but it still uses the generic CAIDBench detector backbone rather than official `IncViTCosine`.
...
Use `method.name=soyo` with `configs/soyo.yaml`. CAIDBench implements per-domain parameter isolation through feature adapters/classifier pools, compresses each domain's normalized frozen features with a `GaussianMixture`, trains a linear SOYO selector from current-domain features plus old GMM-resampled features, and routes inference to the selected domain parameters. The selector path now uses cosine scheduling and old-domain sample counts tied to the current domain size. The official token prompt and per-transformer-layer down/up offset modules remain outside this compact feature-space implementation.
...
Use `method.name=loranpac` with `configs/loranpac.yaml`. CAIDBench follows the official low-rank random-feature solver: random features `H = relu(F @ RE)`, cumulative `cov_HY`, an incremental truncated SVD summary of `H^T`, and on-the-fly ridge weights `U @ ((U.T @ cov_HY) / (Sigma^2 + ridge))`. The local solver now keeps rank according to cumulative sample count, matching the official `round(total_samples * (1 - truncate_percent/100))` behavior. It still uses a compact full-SVD summary update rather than the official QR incremental TSVD implementation, so large-scale memory/runtime characteristics differ.
...
Use `method.name=dce` with `configs/dce.yaml`. CAIDBench implements the DCE expert group as three per-domain expert heads: naive CE, balanced softmax, and reverse/few-shot CE. The compact implementation now uses cosine heads with learnable scale, an MLP selector, CosineAnnealing schedules, and OAS covariance estimates for Gaussian pseudo-feature replay. It does not implement official ViT prompt-token insertion; `prompt_length` remains metadata unless an official DceNet-style backbone is added. The official code's CDDB-Hard imbalance table differs from the paper appendix; reproduce official-code numbers by following `utils/data.py::make_imb()` and recording that choice in experiment metadata.
