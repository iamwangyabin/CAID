...
Use `method.name=cp_prompt` with `configs/cp_prompt.yaml`. CAIDBench implements CP-Prompt as `implementation=feature_space`: a feature-space common prompt plus per-domain personalized prompt, task-center routing, and frozen CLIP/ViT features. It records the official CDDB-Hard values from `configs/prefix_one_prompt/cddb.json`, and it snapshots the common prompt after each task when `is_fix_share_prompt=true`. This is not paper-number equivalent: the official Prefix-One implementation inserts instance tokens into CLIP ViT and injects per-layer K/V prefix prompts, then classifies with per-task CLIP text prompts and `logit_scale`.
